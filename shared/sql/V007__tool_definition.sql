-- ============================================================
--  V007__tool_definition.sql
--  工具定义表（Tool Bus 动态注册支持）
-- ============================================================
...
COMMENT ON COLUMN tool_definition.approval_condition IS '审批触发条件，JSON 格式';
...
-- 查询订单状态 - 低风险只读工具
INSERT INTO tool_definition (name, version, category, description, input_schema, risk_level, requires_approval, enabled)
VALUES ('query_order_status', '1.0', 'query', '查询订单状态',
'{"type": "object", "properties": {"order_id": {"type": "string", "description": "订单 ID"}}, "required": ["order_id"]}',
'low', false, true)
ON CONFLICT (name, version) DO NOTHING;

-- 获取用户信息 - 低风险只读工具
INSERT INTO tool_definition (name, version, category, description, input_schema, risk_level, requires_approval, enabled)
VALUES ('get_user_info', '1.0', 'query', '获取用户基本信息',
'{"type": "object", "properties": {"user_id": {"type": "string", "description": "用户 ID"}}, "required": ["user_id"]}',
'low', false, true)
ON CONFLICT (name, version) DO NOTHING;

-- 模拟写操作 - 高风险工具（需审批）
INSERT INTO tool_definition (name, version, category, description, input_schema, risk_level, requires_approval, approval_condition, enabled)
VALUES ('mock_write_operation', '1.0', 'write', '模拟写操作（高风险）',
'{"type": "object", "properties": {"operation": {"type": "string", "description": "操作类型"}, "amount": {"type": "number", "description": "金额"}}, "required": ["operation"]}',
