...
> **版本**：v1.2 | **状态**：已实施 | **对应审查项**：F-01, F-02, F-03
>
> **实施状态更新（2026-06-25）**：
> - ✅ React + TypeScript 前端（已实现）
> - ✅ Ant Design UI 组件（已集成）
> - ✅ TanStack Router 路由（已配置）
> - ✅ Zustand 状态管理（已实现）
> - ✅ SSE 流式对话（已实现）
...
| 阶段 | 时间 | 交付物 | 验收标准 | 实际状态 |
|------|------|--------|----------|----------|
| **Phase 1: 基础框架** | 第 1-2 周 | 项目搭建、路由、认证、布局 | 登录可用、路由守卫生效 | 🔧 开发中 |
| **Phase 2: 对话功能** | 第 3-4 周 | Chat 界面、SSE 流式、步骤可视化 | 对话流程跑通、流式输出正常 | 🔧 开发中 |
| **Phase 3: 管理功能** | 第 5-6 周 | 审批、工具管理、审计日志 | 管理员可操作、权限控制正确 | 🔧 开发中 |
| **Phase 4: 监控面板** | 第 7-8 周 | Dashboard、图表、实时更新 | 数据展示正确、图表渲染正常 | 🔧 开发中 |
| **Phase 5: 优化上线** | 第 9-10 周 | 性能优化、E2E 测试、部署 | Lighthouse ≥ 90、测试覆盖 ≥ 80% | 🔧 开发中 |
...
| Hook | 用途 | 关键参数 |
|------|------|----------|
| `useDebounce` | 防抖输入 | delay: 300ms |
| `useThrottle` | 节流请求 | limit: 100ms |
| `useLocalStorage` | 本地存储 | key, defaultValue |

### 12.5 核心组件要点

| 组件 | 关键特性 |
|------|----------|
| **InputBox** | Enter发送、Shift+Enter换行、附件上传、最大4000字符 |
| **MessageItem** | 虚拟滚动、代码高亮、复制按钮、步骤折叠 |

### 12.6 审计导出

支持 CSV/JSON 格式导出，前端使用 `json2csv` 库处理。
...
使用 Tailwind `dark:` 变体，通过 `localStorage` 存储用户偏好。
