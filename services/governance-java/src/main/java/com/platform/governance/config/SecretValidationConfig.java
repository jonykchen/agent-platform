...
public class SecretValidationConfig {
    ...
    public void validateSecrets() {
        if (SAFE_PROFILES.contains(activeProfile)) {
            if (serviceAuthSecret.isEmpty()) {
                log.warn("⚠️ SERVICE_AUTH_SECRET is empty in profile '{}'. "
                         + "Set SERVICE_AUTH_SECRET for production.", activeProfile);
            }
            return;
        }

        // 生产/预发环境：强制校验
        if (UNSAFE_SECRETS.contains(serviceAuthSecret)) {
            throw new IllegalStateException(
                    "FATAL: SERVICE_AUTH_SECRET is empty or using a default value in profile '"
                    + activeProfile + "'. Set SERVICE_AUTH_SECRET environment variable.");
        }
        if (serviceAuthSecret.length() < 32) {
            throw new IllegalStateException(
                    "FATAL: SERVICE_AUTH_SECRET must be at least 32 characters in profile '"
                    + activeProfile + "'. Current length: " + serviceAuthSecret.length());
        }
