...
│     ├──[max_consecutive_errors]────────────────────────┼──► final_answer     │
...
【技术选型】interrupt vs 挂起进程
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

┌────────────────────┬─────────────────────────────┬─────────────────────────────┐
│ 方案               │ 优点                        │ 缺点                        │
├────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ LangGraph interrupt│ • 状态完整保存              │ • 需 Checkpoint 支持        │
│ (当前选择)         │ • 可跨服务恢复              │ • 需理解 interrupt 概念     │
│                    │ • 不占用进程资源            │                              │
│                    │ • 支持超时自动恢复          │                              │
├────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ 进程挂起/等待      │ • 实现简单                  │ • 占用进程资源              │
...
├────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ 外部任务队列       │ • 可扩展性好                │ • 架构复杂                  │
│ (Celery/Temporal)  │ • 支持复杂工作流            │ • 需额外服务                │
│                    │                             │ • 与 LangGraph 集成复杂     │
└────────────────────┴─────────────────────────────┴─────────────────────────────┘

【选择 interrupt 的原因】
1. LangGraph 原生支持，与 Checkpoint 机制无缝集成
2. 审批可能跨越数小时甚至数天，进程挂起不可接受
3. 恢复执行时状态完整，无需重新构建上下文
4. 支持超时机制：审批超时可自动取消或提醒

【并发控制原理】
使用 asyncio.Semaphore 限制并发调用：
- MAX_CONCURRENT_MODEL_CALLS = 20：限制模型调用并发
- MAX_CONCURRENT_TOOL_CALLS = 30：限制工具调用并发
- 防止高并发场景下资源耗尽（模型 API 限流、ToolBus 过载）
"""
