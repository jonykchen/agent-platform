...
【核心概念】Token 计数在 Agent 系统中的重要性
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Token 计数用于：
1. 防止超出模型上下文窗口限制（如 128K tokens）
2. 估算 API 调用成本（按 token 计费）
3. 实现上下文截断策略（滑动窗口）
...
│ 方案               │ 优点                        │ 缺点                        │
├────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ tiktoken (选择)    │ • OpenAI 模型精确           │ • 国内模型需映射            │
│                    │ • 性能好                    │ • 额外依赖                  │
│                    │ • 支持 cl100k_base 编码     │                              │
├────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ API usage 字段     │ • 最准确                    │ • 仅调用后可知              │
│                    │ • 包含实际消耗              │ • 无法用于预截断            │
├────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ 字符数近似         │ • 无依赖                    │ • 不准确（误差 20-50%）     │
│                    │ • 最快                      │ • 中英文差异大              │
├────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ HuggingFace Tokenizers│ • 支持多模型              │ • 依赖重                    │
│                    │ • 开源模型精确              │ • 加载慢                    │
└────────────────────┴─────────────────────────────┴─────────────────────────────┘

【选择 tiktoken + 近似回退的原因】
1. tiktoken 对 OpenAI 模型精确，国内模型（Qwen/GLM）结构类似，可近似
2. 回退方案保证无依赖时也能工作（开发环境）
3. 性能平衡：tiktoken 编码速度快，适合高频调用

【国内模型适配策略】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

国内模型（Qwen、GLM、DeepSeek）没有官方 tokenizer，采用映射策略：

┌─────────────────────────────────────────────────────────────────────────────┐
│  模型          │  映射到         │  原因                                     │
├────────────────┼─────────────────┼───────────────────────────────────────────┤
│  qwen-*        │  cl100k_base   │  通义千问与 GPT-4 tokenizer 结构相似      │
│  glm-*         │  cl100k_base   │  智谱 GLM 使用类似 BPE 编码               │
│  deepseek-*    │  cl100k_base   │  DeepSeek 基于 GPT 架构                   │
└────────────────┴─────────────────┴───────────────────────────────────────────┘

注意：映射是近似值，实际 token 数可能有 5-10% 误差。
生产环境建议使用 API 返回的 usage 字段进行校准。

【近似算法原理】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

当 tiktoken 不可用时，使用字符数近似：

- 中文字符：字符数 / 1.5（约 1.5 字符/token）
- 英文/数字：字符数 / 4.0（约 4 字符/token）

示例：
- "你好世界" → 4 字符 → 约 2.67 tokens（实际约 3）
- "Hello World" → 11 字符 → 约 2.75 tokens（实际约 2）
